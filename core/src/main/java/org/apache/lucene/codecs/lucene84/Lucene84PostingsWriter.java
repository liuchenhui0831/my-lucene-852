...
public final class Lucene84PostingsWriter extends PushPostingsWriterBase {
  ...
  public void startDoc(int docID, int termDocFreq) throws IOException {
    ...
    if (lastBlockDocID != -1 && docBufferUpto == 0) {
      // 到一个块大小，生成跳表，以便读取阶段快速跳转到指定的PackedBlock。
    ...
    docDeltaBuffer[docBufferUpto] = docDelta; // 将docID 差值存储
    if (writeFreqs) {
    ...
    //足够一个块大小，即128个文档，存储其文档号，然后以PackInts格式存储词频
  ...
  public void addPosition(int position, BytesRef payload, int startOffset, int endOffset) throws IOException {
    ...
    posDeltaBuffer[posBufferUpto] = position - lastPosition; //term在一个文档中的位置是有序的，可以差值存储
    if (writePayloads) {
    ...
    if (writeOffsets) {
      ...
      offsetStartDeltaBuffer[posBufferUpto] = startOffset - lastStartOffset; //startOffset也是有序的，可以差值存储
    ...
    if (posBufferUpto == BLOCK_SIZE) {  //一个块大小
  ...
  public void finishDoc() throws IOException {
    ...
    if (docBufferUpto == BLOCK_SIZE) { //足够一个块，记录一些上一个块的信息
  ...
  public void finishTerm(BlockTermState _state) throws IOException {
    ...
    if (state.docFreq == 1) {
      ...
      for(int i=0;i<docBufferUpto;i++) {  //这个term有剩余这么多个文档未处理，单独用几个vint表示一个doc的信息
