 ...
 *
 * 在多线程（持有相同IndexWriter对象的引用）执行删除操作时，多个DWPT将自己携带的删除信息
 * 生成Node对象，并添加到deleteQueue中，deleteQueue是一个全局的变量。
 */
final class DocumentsWriterDeleteQueue implements Accountable, Closeable {
  ...
  /**
   * 每个DWPT都拥有一个私有的DeleteSlice对象，初始化时，会让sliceHead=sliceTail=tail，即都指向最新的一个删除操作。
   */
