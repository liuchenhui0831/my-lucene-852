...
public class ConcurrentMergeScheduler extends MergeScheduler {
  ...
  /**
   * 限制生成新的OneMergeThread前，允许最大OneMergeThread的个数
   */
   ...
   *
   * 在doAutoIOThrottle = true的情况下，被节流的OneMergeThread最少能保证写入5M，
   * 但如果OneMergeThread中的OneMerge大小OneMergeSize ＜ MIN_BIG_MERGE_MB，那么
   * 不会受到合并线程的I/O节流的限制，即Double.POSITIVE_INFINITY。
   */
   ...
   *
   * 在doAutoIOThrottle = true的情况下，被节流的OneMergeThread最多允许写入10G
   */
   ...
   *
   * 用来定义OneMerge是否为一个大合并(big Merge)，即判断OneMerge中所有段的索引大小，我们称之为OneMergeSize，总和是否超过50M
   */
  private static final double MIN_BIG_MERGE_MB = 50.0;

  /** Current IO writes throttle rate
   *
   * 最初的值为START_MB_PER_SEC，在doAutoIOThrottle = true的情况下，刚刚创建结束
   * 的OneMergeThread的磁盘的写入量会被置为targetMBPerSec，而targetMBPerSec的值在
   * 每一次新增一个OneMergeThread后发生变化，或变大或减少，在合并线程的I/O节流下也
   * 会用到该值，影响正在运行的OneMergeThread的最大磁盘可写入量。
   */
  protected double targetMBPerSec = START_MB_PER_SEC;

  /** true if we should rate-limit writes for each merge
   *
   *  是否需要对每一个合并线程OneMergeThread执行合并线程的I/O节流，OneMergeThread在创建时，
   *  它允许写入磁盘的大小是Double.POSITIVE_INFINITY，即没有写入数量的限制，而在doAutoIOThrottle
   *  为true的情况下，磁盘的写入量在运行期间可能会被修改多次。
   */
  private boolean doAutoIOThrottle = true;

  /**
   * 默认值表示：IndexWriter执行forceMerge时，即使OneMerge是一个大合并，也不会受到合并线程的I/O节流的限制。
   */
  ...
  protected synchronized void updateMergeThreads() {
    ...
    CollectionUtil.timSort(activeMerges); //按这个OneMerge所涉及的大小排序
    ...
    for (threadIdx=activeMergeCount-1;threadIdx>=0;threadIdx--) {
      ...
      if (mergeThread.merge.estimatedMergeBytes > MIN_BIG_MERGE_MB*1024*1024) {
        bigMergeCount = 1+threadIdx;  //统计出大合并
        break;
      }
    }
    ...
    for (threadIdx=0;threadIdx<activeMergeCount;threadIdx++) {  //前面排过序了
      MergeThread mergeThread = activeMerges.get(threadIdx);

      OneMerge merge = mergeThread.merge;

      // pause the thread if maxThreadCount is smaller than the number of merge threads.
      final boolean doPause = threadIdx < bigMergeCount - maxThreadCount; //前面超过限制的这部分大合并线程 为true,即让其磁盘写入为0

      double newMBPerSec;  //新的最大磁盘写入量
      if (doPause) {
        newMBPerSec = 0.0;
      } else if (merge.maxNumSegments != -1) {
        newMBPerSec = forceMergeMBPerSec;  //这是个forceMerge, 不受限制
      } else if (doAutoIOThrottle == false) {
        newMBPerSec = Double.POSITIVE_INFINITY;  //不受自动节流限制
  ...
  protected synchronized boolean maybeStall(IndexWriter writer) {
    ...
    //说明有maxMergeCount-maxThreadCount个线程处于暂停状态
    while (writer.hasPendingMerges() && mergeThreadCount() >= maxMergeCount) {
      ...
      if (mergeThreads.contains(Thread.currentThread())) {
        ...
        return false; //合并让当前线程退出，从而减少新的Thread生成
      }
