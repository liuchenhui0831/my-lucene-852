...
public abstract class MergePolicy {
   ...
   *  non-compound format
   *
   * 新段的大小mergedInfoSize占索引目录中所有段的大小的百分比如果超过该值，那么就不使用复合索引文件
   */
 ...
 *          produced by a cascaded merge.
  ...
  public boolean useCompoundFile(SegmentInfos infos, SegmentCommitInfo mergedInfo, MergeContext mergeContext) throws IOException {
    ...
    //合并后的新段的大小，根据不同的合并策略，mergedInfoSize的含义各不相同
    //- LogByteSizeMergePolicy：mergedInfoSize的值为新段对应的每一个索引文件的大小和值，通过JDK提供
    //          的File.size(Path)的方法来计算单个索引文件的大小。注意的是该合并策略还提供了setCalibrateSizeByDeletes()
    //          方法来让用户选择mergedInfoSize的值是否要剔除被删除的文档的索引大小;
    //- LogDocMergePolicy：mergedInfoSize的值为新段中文档的数量maxDoc，该合并策略也提供了setCalibrateSizeByDeletes()
    //          方法来让用户选择mergedInfoSize的值是否要剔除被删除的文档的数量
