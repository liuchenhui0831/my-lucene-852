 ...
 * 保存了删除信息
 *
 * 一个DWPT从生成到flush到一个段，在这段期间其私有的DeleteSlice会记录所有的删除操作，这些删除操作会
 * 作用于(apply)DWPT添加的文档，即删除那些满足删除要求的文档，然而对于某一个删除操作来说，它只能作用
 * 于该删除操作前DWPT已添加的文档，其随后新添加的文档不能被apply，BufferedUpdates类就是通过以下几个
 * Map对象来描述 某一个删除操作的作用范围(apply scope)：
 */
class BufferedUpdates implements Accountable {
  ...
  /**
   * 该Map的key为Term，表示包含该Term的文档都会被删除，value为一个哨兵值，描述了该删除操作的
   * 作用范围，即只能作用于文档号小于哨兵值的文档;
   *
   * 在DWPT将索引信息生成索引文件期间，利用倒排表中的信息找到包含该Term的所有文档的文档号，文档号
   * 小于哨兵值的文档都会删除，当然所谓的""删除""其实是将被删除的文档号从文档号集合
   * （docId Set，0 ~ (numDocsInRAM - 1)的文档号集合）中剔除，在处理完所有的
   * 删除(比如下文中的deleteQueries)后，该集合会生成索引文件.liv.
   */
  final Map<Term,Integer> deleteTerms = new HashMap<>(); // TODO cut this over to FieldUpdatesBuffer
  /**
   * Map的key为Query，表示满足该查询要求的文档都会被删除，value的概念同deleteTerms。
   *
   * 在DWPT将索引信息生成索引文件之后，通过查询的方式找出满足删除要求的文档号，然后从文档号集合中剔除这些文档号.
   */
