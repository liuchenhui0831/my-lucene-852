...
final class IndexFileDeleter implements Closeable {
  ...
  public IndexFileDeleter(String[] files, Directory directoryOrig, Directory directory, IndexDeletionPolicy policy, SegmentInfos segmentInfos,
    ...
    if (currentSegmentsFile != null) {
      ...
      for (String fileName : files) {
        m.reset(fileName);
        //判断文件是否满足要求， pending_segments文件时两阶段commit中第一阶段生成的文件
        if (!fileName.endsWith("write.lock") && (m.matches() || fileName.startsWith(IndexFileNames.SEGMENTS) || fileName.startsWith(IndexFileNames.PENDING_SEGMENTS))) {
          ...
          getRefCount(fileName); //初始化索引文件计数
          //segments文件特殊处理
          if (fileName.startsWith(IndexFileNames.SEGMENTS) && !fileName.equals(IndexFileNames.OLD_SEGMENTS_GEN)) {
            ...
            //如果读到的SegmentInfos和传给构造方法的generation是一样的
            //因为有可能保留多个Segment_N，但是只有一个和传入的一样
            if (sis.getGeneration() == segmentInfos.getGeneration()) {
              currentCommitPoint = commitPoint;
            }
            commits.add(commitPoint);
            //增加每一个segments_N对应的SegmentInfos中对应的索引文件的计数引用，
            //其原因是在后面的流程中能判断能否能删除索引文件。
            incRef(sis, true);
    ...
    //循环了所有的文件后，还是没有读取到Segment_N(currentCommitPoint==null)，可能是NFS导致，
    // 如目录缓存机制的影响，通过重试重新获取SegmentInfos对象。
    ...
    if (isReaderInit) {
      ...
      //这里isCommit==false, 目的就只是为了增加segmentInfos对应索引文件的计数。原因是，如果这个是
      // 从IndexCommit初始化来的，即NRT方式获取到的。这样最后一个段的文件可能没有记录到段文件中，
      // 如果不增加计数的话可能会被删掉。
      checkpoint(segmentInfos, false);
    }

    // We keep commits list in sorted order (oldest to newest):
    //因为之前添加到commits中的顺序是无序的，如果是KeepOnlyLastCommitDeletionPolicy的
    // 删除策略的话，可能就无法处理，所以这里要按照从旧到新的提交顺序排序。
    ...
    //之前的IndexWriter没有“优雅”的退出（操作系统/物理机 崩溃或者断电），导致索引目录中生成了
    // 一些“不优雅”的文件，为了避免新的IndexWriter生成的索引文件的文件名与索引目录中的相同（即double-write）
    // 可能会引起一些问题，不如先根据索引目录中索引文件找到gens的各自的最大值N，使得随后生成的索引文件的迭代编号从N+1开始。
    //"不优雅的文件"就比如commit了第一阶段，有文件，但是只有pending_segment文件，下次加载起来的时候可能导致序号和之前的段重复。
  ...
  public void checkpoint(SegmentInfos segmentInfos, boolean isCommit) throws IOException {
    ...
    // Incref the files:
    //增加段文件使用的索引文件的引用，保证这些文件不会在后面的删除操作中被删除
    incRef(segmentInfos, isCommit);

    if (isCommit) {  //是在commit()过程中调用，incRef()中增加这次提交的索引文件的计数引用。在if分支中执行索引删除策略；
      ...
      deleteCommits();
    } else {  //尝试删除lastFiles中的索引文件，再重新将这次SegmentInfo对应的索引文件添加到lastFiles中；
      // DecRef old files from the last checkpoint, if any:
      try {
        decRef(lastFiles);
      } finally {
