...
final class DefaultIndexingChain extends DocConsumer {
  ...
  public Sorter.DocMap flush(SegmentWriteState state) throws IOException {
    ...
    writeNorms(state, sortMap);  // nvd  nvm
  ...
  public void processDocument() throws IOException {
    ...
    try {
      for (IndexableField field : docState.doc) { //处理当前文档的每个域
      ...
      if (docWriter.hasHitAbortingException() == false) {
        ...
        for (int i=0;i<fieldCount;i++) {
          fields[i].finish(); //处理存储域的norm
        }
        finishStoredFields(); //增量统计存储域信息
      }
    }
  ...
  private PerField getOrAddField(String name, IndexableFieldType fieldType, boolean invert) {
    ...
    final int hashPos = name.hashCode() & hashMask;
    PerField fp = fieldHash[hashPos]; //hash表，碰撞后while循环
    while (fp != null && !fp.fieldInfo.name.equals(name)) {
      fp = fp.next;
    }
    ...
    if (fp == null) {
      ...
      fieldHash[hashPos] = fp; //加入hash链表中
      totalFieldCount++;
      ...
      try (TokenStream stream = tokenStream = field.tokenStream(docState.analyzer, tokenStream)) {
        ...
        while (stream.incrementToken()) {
          ...
          try {
            ...
            //具体值通过fieldState.termAttribute在TermsHashPerField.start()时候传入了
            //只要在add()时候获取就可以
