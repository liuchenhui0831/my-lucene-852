...
/**
 * 用来保留提交的快照，它封装了其他的索引删除策略，由于NoDeletionPolicy保留了每一次的提交，
 * 所以封装该策略没有什么意义，当封装了KeepOnlyLastCommitDeletionPolicy，那么可以通过主动
 * 调用SnapshotDeletionPolicy.snapshot()的方法来实现快照功能，使得新的提交产生后，上一个
 * 提交能以快照的方式保留在内存中，这种策略的缺点在于需要额外一份索引信息大小的内存。
 *
