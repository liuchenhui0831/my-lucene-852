...
class PendingDeletes {
   ...
   *
   * 判断要删除的文档是不是就是整个段的文档数，即整个段要不要丢弃
   */
   ...
   *
   * - delCount  该值描述的是满足删除信息TermArrayNode、TermNode的文档的个数，在生成索引
   *           文件.tim、.tip、.doc、.pos、.pay的过程中会找到那些满足删除要求的文档号，随后将
   *           这些文档号添加到FixedBitSet对象中，随后FixedBitSet中的文档信息在写入到索引文
   *           件.liv过程中，将被删除的文档的个数统计到SegmentCommitInfo的delCount中;
   * - softDelCount  该值描述的是包含软删除信息的文档;
   * - maxDoc：该值描述了当前段中的文档总数，即DWPT收集的文档的文档个数;
   */
