 ...
 * </p>
 *
 * DocumentsWriterPerThreadPool是一个逻辑上的线程池，它实现了类似Java线程池的功能。每当IndexWriter要添加
 * 文档，会从DocumentsWriterPerThreadPool中获得一个ThreadState去执行，故在多线程（持有相同的IndexWriter
 * 对象引用）执行添加文档操作时，每个线程都会获得一个ThreadState对象。
 *
 * 每一个ThreadState对象中都持有一个DWPT的引用，所以正如上文中所述，实际的添加文档操作还是DWPT。
 * 当ThreadState执行完添加文档的任务后，它会回到DocumentsWriterPerThreadPool中，等待下次的文档添
 * 加操作，通过一个名为freeList的List来存储。
 *
 * ThreadState在两种情况下不持有DWPT的引用:
 *  1. 新的添加文档的任务，这个时候DWPT-pool中没有可用的ThreadState，就会生成一个新的ThreadState，但是没有DWPT;
 *  2. DWPT在执行完添加文档的操作后，在某个场景下ThreadState会被标记为flushPending状态，失去该DWPT的引用，随
 *     后DWPT执行doFlush操作，将收集到的索引信息生成索引文件；
 */
