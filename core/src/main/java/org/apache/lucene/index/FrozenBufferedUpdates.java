...
final class FrozenBufferedUpdates {
   ...
   *  判断其他线程是否已经处理过当前删除信息，即applied==0
   */
    ...
    try {
      ...
      while (true) {
        ...
        synchronized (writer) {
          ...
          segStates = openSegmentStates(writer, infos, seenSegments, delGen()); ///获取SegmentState
          ...
          writer.deleter.incRef(delFiles); //防止文件在其他线程合并完文件后删除 **
        }
        ...
        try (Closeable finalizer = () -> finishApply(writer, segStates, success.get(), delFiles)) {
          ...
          delCount = apply(segStates); //具体处理几种Delete Node
          success.set(true);
        }
        ...
        synchronized (writer) {
          ...
          //说明从获取到mergeGenStart到现在没有其他线程执行段合并，或者某些段合并还没结束
          //不相等的话说明这时目录中的段已经发生了变化，需要重新将全局的FrozenBufferedUpdates作用到索引目录中的段
  ...
  private static BufferedUpdatesStream.SegmentState[] openSegmentStates(IndexWriter writer, List<SegmentCommitInfo> infos,
    ...
    try {
      for (SegmentCommitInfo info : infos) {
        //bufferedDeletesGen  描述SegmentCommitInfo的delGen,delGen的值即nextGen;
        //delGen: 即当前FrozenBufferedUpdates的nextGen
        //<= 即删除信息要作用到所有比delGen小的段
        //
  ...
  private long apply(BufferedUpdatesStream.SegmentState[] segStates) throws IOException {
    ...
    totalDelCount += applyTermDeletes(segStates); //处理TermDelete
    totalDelCount += applyQueryDeletes(segStates); // 处理QueryDelete
    totalDelCount += applyDocValuesUpdates(segStates); // 处理DocValuesUpdates
