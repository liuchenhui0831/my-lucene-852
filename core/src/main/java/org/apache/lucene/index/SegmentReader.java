...
public final class SegmentReader extends CodecReader {
  ...
  /**
   * 描述的是段中不变的信息（StoredFieldsReader、TermVectorsReader、PointsReader、NormsProducer），
   * 可以复用。如果在之后的操作中，段中的索引信息发生更改，更改的索引信息会以其他的索引文件来描述，即
   *  liv,dvm,dvd,fnm,如文档删除，docValue的更新。所以在DirectoryReader.openIfChange()获取最新
   *  的StandardDirectoryReader时，即使多个SegmentReader发生变更，还是可以直接复用SegmentCoreReader中
   *  的索引信息，只需要更新相对开销较小的liv,dvm,dvd,fnm即可；
   */
  ...
  final FieldInfos fieldInfos; //可能有DocValue导致的域名有变化，需要在core之外单独记一个
  ...
  /**
   * 如果有DocValue类型导致索引变化，需要重新读取.fnm文件
   *
  ...
  private FieldInfos initFieldInfos() throws IOException {
    if (!si.hasFieldUpdates()) {
      ...
      return fisFormat.read(si.info.dir, si.info, segmentSuffix, IOContext.READONCE); //域名有更新的话，可能文件名为_0_1.fnm
    }
  }
