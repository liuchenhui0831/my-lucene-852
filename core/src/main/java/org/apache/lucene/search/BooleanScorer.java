...
final class BooleanScorer extends BulkScorer {
  ...
  static class Bucket {
    double score;  // 某个文档的打分
    int freq;   // 这个文档号出现的次数，用来跟minShouldMatch比较，判断是否满足要求
  }
  ...
  private class BulkScorerAndDoc {
    ...
    BulkScorerAndDoc(BulkScorer scorer) {
      this.scorer = scorer;  // 存放某个域值对应的BulkScorer对象
      this.cost = scorer.cost();  // 包含这个域值的文档个数
      this.next = -1;
    }
    // 找到下一个不小于min的文档号
  ...
  final HeadPriorityQueue head;   // 存放那些包含某个域值的文档个数相对较少的BulkScorerAndDoc对象
  final TailPriorityQueue tail;   // 存放那些包含某个域值的文档个数相对较多的BulkScorerAndDoc对象
  ...
  final class OrCollector implements LeafCollector {
    ...
    public void collect(int doc) throws IOException {
      ...
      bucket.freq++;
      bucket.score += scorer.score();  // 这里可以看出，打分是一个累加的过程
    }
  }
  ...
  private void scoreDocument(LeafCollector collector, int base, int i) throws IOException {
    ...
    final Bucket bucket = buckets[i];
    if (bucket.freq >= minShouldMatch) {  // if语句为true：文档号出现的次数满足minShouldMatch
      scoreAndDoc.score = (float) bucket.score;
      final int doc = base | i;  // 根据之前得到的windowBase值，恢复文档号真正的值
      scoreAndDoc.doc = doc;
  ...
  private void scoreMatches(LeafCollector collector, int base) throws IOException {
    ...
    for (int idx = 0; idx < matching.length; idx++) {
      ...
      while (bits != 0L) {  // 反序列化的过程，得到所有的文档号
  ...
  private void scoreWindowIntoBitSetAndReplay(LeafCollector collector, Bits acceptDocs,
    ...
    for (int i = 0; i < numScorers; ++i) {   // 遍历每一个BulkScorerAndDoc，统计文档号出现的次数跟打分
  ...
  private BulkScorerAndDoc advance(int min) throws IOException {
    ...
    // 每次更新一个head中的对象后就进行调整堆，当headTop.next都不小于min，说明优先级队列中所有的对象
    //     的next都更新到了不小于min的值（查看lessThan的比较规则）
    while (headTop.next < min) {
      if (tailTop == null || headTop.cost <= tailTop.cost) {
        headTop.advance(min);  // 查询倒排表，或者下一个文档号
        headTop = head.updateTop();   // 调整堆（最小堆）
      } else {
  ...
  private void scoreWindowMultipleScorers(LeafCollector collector, Bits acceptDocs, int windowBase, int windowMin, int windowMax, int maxFreq) throws IOException {
    ...
    while (maxFreq < minShouldMatch && maxFreq + tail.size() >= minShouldMatch) {
      ...
      if (candidate.next < windowMax) {
        ...
        head.add(candidate);  // 如果next值大于windowMax，那么放到this.head, 等待下次再处理
      }
    }
  ...
  private BulkScorerAndDoc scoreWindow(BulkScorerAndDoc top, LeafCollector collector,
    ...
    leads[0] = head.pop();  // next值最小的BulkScorerAndDoc对象放到leads[]数组中的第一个第一个位置
    int maxFreq = 1;
