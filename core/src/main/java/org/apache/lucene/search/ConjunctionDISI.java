...
public final class ConjunctionDISI extends DocIdSetIterator {
  ...
  final DocIdSetIterator[] others;  // others数组中所有对象的cost值都不小于lead1跟lead2中的cost值
  ...
  private int doNext(int doc) throws IOException {
    advanceHead: for(;;) {
      assert doc == lead1.docID(); // 取出当前正在使用lead1的文档号(文档号总是从最小的开始遍历)
      ...
      final int next2 = lead2.advance(doc); // 取出当前正在使用的lead2的文档号(文档号总是从最小的开始遍历)
      if (next2 != doc) {
        doc = lead1.advance(next2);  // 取出lead1的下一个文档号
        if (next2 != doc) {
      ...
      for (DocIdSetIterator other : others) {
        ...
        if (other.docID() < doc) {   // 注意的是每一个other.docID的第一次调用都是-1,并且之后的调用都是不小于doc的值
          final int next = other.advance(doc);

          if (next > doc) {
            // iterator beyond the current doc - advance lead and continue to the new highest doc.
            doc = lead1.advance(next);   // 当前的doc不满足，那么另doc为下一个不小于next的值
