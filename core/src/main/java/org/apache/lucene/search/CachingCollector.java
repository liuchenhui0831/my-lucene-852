...
/**
 * 缓存Collector收集的一次搜索的结果，使得其他的Collector可以复用该Collector的数据。在缓存阶段，
 * 设置了最大缓存个数，超过后，就会清空缓存的所有数据，置位标志变量。缓存了：
 * - List<LeafReaderContext>:一个LeafReaderContext描述的是一个段内的信息，多个段就是List存储；
 * - List<int[]> docs：一个段中可能有多个满足查询条件的文档，使用int[]来缓存这些文档的文档号；
 * - List<float[]> socres: 一个段中所有满足查询条件的文档的打分值使用float[]缓存；
 *
 * 可以设置最大缓存文档数，超过阈值会清空缓存的所有数据，让变量cache=false, 令缓存操作失败，复用时要注意检查。
 *
