...
/**
 * 封装了其他Collector,用来限制Collector处理文档的时间。即设定了一次查询允许的最长时间timeLimit。
 * 如果查询的时间超过timeLimit，那么会抛出超时异常TimeExceededException。
 * 贪婪模式下，超时时会先收集再抛出。
 *
 * 在getLeafCollector()和LeafCollector.collect()中会判断超时。
 * 通过后台线程实现超时机制。
 *
 * 这种方法可能精度不是特别高，而且上下文切换导致的性能不是特别好。
 *
...
public class TimeLimitingCollector implements Collector {
  ...
  public static final class TimerThread extends Thread  {
    ...
    private volatile long resolution;  //精度，ms，间隔多久会再次判断时间
