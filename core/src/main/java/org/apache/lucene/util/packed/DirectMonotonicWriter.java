...
public final class DirectMonotonicWriter {
  ...
  private void flush() throws IOException {
    ...
    for (int i = 0; i < bufferSize; ++i) {
      ...
      buffer[i] -= expected;  //缩放，将大的值降下来，使得最大元素的bit位数降低
    }
    //由于在第一次缩放数据的过程中，buffer[ ]数组中的元素可能不再是递增的了，所以只能通过逐个遍历数组元素来找到最小值
    long min = buffer[0];
    for (int i = 1; i < bufferSize; ++i) {
      min = Math.min(buffer[i], min);
    }

    //在缩放数据后，buffer中的有些数据变成了负数，由于随后将会使用PackedInts存储，
    // 该方法只支持存储无符号的数值，故结合上一步中获得的最小值min，通过下面的方法使得所有的数值进行无符号处理。
    long maxDelta = 0;
    for (int i = 0; i < bufferSize; ++i) {
      buffer[i] -= min;
    ...
    // offset 每1024个数据就生成一个block，当生成多个block时，通过记录offset来描述每一个block的起始读取位置
