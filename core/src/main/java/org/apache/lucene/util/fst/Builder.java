...
public class Builder<T> {
  ...
  // current "frontier"
  private UnCompiledNode<T>[] frontier; // 存放状态为UnCompiledNode的Node的数组
  ...
  */
  ...
  public void add(IntsRef input, T output) throws IOException {
    ...
    */
    ...
    final int pos1Stop = Math.min(lastInput.length(), input.length); //根据上次长度和本次输入长度找到最长需要比较前缀相同的长度位置，即min
    while(true) {
      frontier[pos1].inputCount++;
      //System.out.println("  incr " + pos1 + " ct=" + frontier[pos1].inputCount + " n=" + frontier[pos1]);
      if (pos1 >= pos1Stop || lastInput.intAt(pos1) != input.ints[pos2]) { //前缀挨个比较，找到最长相同前缀处
        break;
      }
      pos1++;
      pos2++;
    }
    final int prefixLenPlus1 = pos1+1; //确定具有公共前缀的位置
    //新插入的节点放到frontier数组，unCompiledNode表明是新插入的，以后还可能会变化，
    //还未放入FST内
    ...
    // orphan'd suffix
    freezeTail(prefixLenPlus1); //从prefixLenPlush1，进行freeze冰冻操作，添加并构建最小FST

    // init tail states for current input
    //将当前input剩下的部分插入，构建arc转移（前缀是共用的，不用添加新的状态）
    ...
    for(int idx=1;idx<prefixLenPlus1;idx++) {
      ...
      if (lastOutput != NO_OUTPUT) {
        //使用common方法，计算output的共同前缀， 两者中最小值
        commonOutputPrefix = fst.outputs.common(output, lastOutput);
        assert validOutput(commonOutputPrefix);
        //使用subtract方法，计算重新分配的output
        wordSuffix = fst.outputs.subtract(lastOutput, commonOutputPrefix); // 差值
  ...
  public static class Arc<T> {
    public int label;    // 节点值     // really an "unsigned" byte
    public Node target;  //目标节点
    public boolean isFinal;
    public T output; // payload 附加值
