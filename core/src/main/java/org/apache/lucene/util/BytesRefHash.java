...
/**
 * 为BytesRef对象优化的一种类似HashMap的数据结构，是将所有的BytesRef对象存储到一个连续的
 * 存储空间，使得能在查询阶段达到O(1)的时间复杂度。
 * 利用多余的2个数组，通过hash找到term所在的位置。
 *
...
public final class BytesRefHash implements Accountable {
  ...
  int[] bytesStart; //下标是termID,数组元素是termID对应的BytesRef值的起始位置
  ...
  private int[] ids; //下标是BytesRef对利用MurmurHash算法计算出的hash值，元素是termID
