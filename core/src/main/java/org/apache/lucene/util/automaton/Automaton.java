...
public class Automaton implements Accountable {
  ...
  //固定连续2个数组元素描述当前状态的两个信息[当前状态第一个转移函数在transitions数组中的起始位置，当前状态转移函数的数量]
  ...
  private int[] transitions; //用固定的连续3个元素描述一个转移函数的3个信息[目标状态，输入符号min,输入符号max]
  ...
  public void addTransition(int source, int dest, int min, int max) {
    ...
    growTransitions();
    if (curState != source) {
      if (curState != -1) { //不是初始值，已经添加过转换的话要优化合并上次的转换
        finishCurrentState();
      }
      ...
      states[2*curState] = nextTransition; //记录位置
    }
