 ...
 *
 * 提供了很多对整数(integers)的压缩存储方法，其压缩存储过程其实就是对数据进行编码，
 * 将每一个整数（long或者int）编码为固定大小进行存储，大小取决于最大的那个值所需要的bit位个数。
 * 优点是减少了存储空间，并且对编码后的数据能够提供随机访问的功能。
 */
  ...
  public void encode(long[] values, int valuesOffset, byte[] blocks,
    ...
    for (int i = 0; i < byteValueCount * iterations; ++i) {
      ...
      if (bitsPerValue < bitsLeft) {
        ...
        while (bits >= 8) {  // 每个元素需要的bit位的个数大于8时的处理
          bits -= 8;
          blocks[blocksOffset++] = (byte) (v >>> bits); // 将一个元素分成多块(按照一个8个bit大小划分)存储
        }
        // then buffer
        bitsLeft = 8 - bits;
        // 把v的值的剩余部分存放到下一个nextBlock中,也就是当前的v值的部分值会
        // 跟下一个v值的数据(可能是部分数据)混合存储到同一个字节中
